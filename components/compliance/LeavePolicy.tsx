...
const PolicyCard: React.FC<{ title: string; children: React.ReactNode }> = ({ title, children }) => (
    <div className="bg-secondary p-6 rounded-lg border border-border">
        <h4 className="text-md font-bold text-blue-500 dark:text-blue-400 mb-3">{title}</h4>
        <div className="text-sm text-card-foreground space-y-2">{children}</div>
    </div>
);

const LeavePolicy: React.FC = () => {
    return (
        <div className="bg-card text-card-foreground p-6 rounded-lg shadow-md max-w-4xl mx-auto border border-border">
            <h3 className="text-lg font-semibold text-foreground mb-4">Leave Policy Overview (Qatar Labor Law)</h3>
            <p className="text-sm text-muted-foreground mb-6">This is a simplified summary for reference purposes. Always consult the official Qatar Labor Law for complete and up-to-date information.</p>

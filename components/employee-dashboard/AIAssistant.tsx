        ...
        <div className="bg-brand-light p-4 rounded-lg shadow-md h-full flex flex-col">
            ...
            <div className="flex-grow overflow-y-auto pr-2 mb-4 space-y-4 h-64">
                {messages.map(msg => (
                    <div key={msg.id} className={`flex items-end gap-2 ${msg.sender === 'user' ? 'justify-end' : 'justify-start'}`}>
                        ...
                        <div className={`px-4 py-2 rounded-xl max-w-xs ${msg.sender === 'user' ? 'bg-brand-primary text-white rounded-br-none' : 'bg-brand-secondary text-nav-text rounded-bl-none'}`}>
                 ...
                 {isLoading && (
                    <div className="flex items-end gap-2 justify-start">
                        ...
                        <div className="px-4 py-2 rounded-xl bg-brand-secondary text-nav-text rounded-bl-none">
                            <div className="flex items-center space-x-1">
                                <span className="w-2 h-2 bg-nav-icon rounded-full animate-bounce delay-0"></span>
                                <span className="w-2 h-2 bg-nav-icon rounded-full animate-bounce delay-150"></span>
                                <span className="w-2 h-2 bg-nav-icon rounded-full animate-bounce delay-300"></span>
                            </div>
                        </div>
                    </div>
                 )}
            ...
            <div className="flex items-center border-t border-slate-700 pt-2">
                <input
                    ...
                    className="flex-grow bg-brand-secondary border border-slate-600 rounded-lg p-2 text-sm text-brand-dark focus:ring-brand-primary focus:border-brand-primary"
                    disabled={isLoading}
                />

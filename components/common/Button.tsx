...
const Button: React.FC<ButtonProps> = ({ children, variant = 'primary', isLoading = false, icon, className = '', size = 'md', ...props }) => {
  ...
  const variantClasses = {
    ...
    secondary: 'bg-brand-light text-brand-dark border border-slate-300 hover:bg-slate-50 focus:ring-brand-primary/50',
  ...
  return (
    ...
    >
      {isLoading ? (
        <svg data-testid="loading-spinner" className="animate-spin -ml-1 mr-3 h-5 w-5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
